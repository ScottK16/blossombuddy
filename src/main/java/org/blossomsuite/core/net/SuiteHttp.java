...
public final class SuiteHttp {
   ...
   private static final String VERIFY_PREFIX = "/v1/verify/";
   ...
   /**
    * The only requests this mod will ever send to a relay: vote-party sync, cross-realm chat, the usage counts, the player list,
    * emotes, and linking a Discord account. Map art never touches a relay at all - a design is downloaded from the website and read straight off disk. Everything else
   ...
   public static boolean isAllowedPath(String path) {
      return path != null && (path.startsWith(VOTE_PREFIX) || path.startsWith(CHAT_PREFIX) || path.startsWith(STATS_PREFIX) || path.startsWith(PRESENCE_PREFIX) || path.startsWith(EMOTE_PREFIX) || path.startsWith(VERIFY_PREFIX));
   }
